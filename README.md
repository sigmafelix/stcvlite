# stcvlite <img align="right" width="250" src="man/figures/logo.png">

  <!-- badges: start -->
  [![R-CMD-check](https://github.com/sigmafelix/stcvlite/actions/workflows/R-CMD-check.yaml/badge.svg)](https://github.com/sigmafelix/stcvlite/actions/workflows/R-CMD-check.yaml)
  <!-- badges: end -->
...
`stcvlite` provides convenient functions to make `rset` objects for spatiotemporal cross-validation. `rset` class is a major component of `rsample` package, which is a part of `tidymodels` ecosystem, to perform hyperparameter tuning and model evaluation for machine learning tasks. 

## Cross-validation schemes
The package offers several cross-validation schemes by _what to split along_, for example, leave-one-out or leave-block-out along space or time indices or by combination of both. The principle encapsulates the schemes as acronyms. To note, the terminology is from [Clark et al. (2024)](https://doi.org/10.1038/s41370-024-00712-8):

| Family | Scheme | Description |
|:------:|:------:|:----------------|
...
| Random | `"random"` | Random assignment. Same as plain random CV. |

## Installation

```r
# install.packages("pak")
pak::pak("sigmafelix/stcvlite")
```
...
Spatial blocks group nearby locations together so that an entire block ends up in
the same fold, preventing spatial autocorrelation from leaking between training and
test sets. `generate_block_sp_index()` attaches a `sp_index` column to the covariate
table and supports three block-definition strategies.
...
Pass a length-2 numeric vector to cut the bounding box into rectangular cells.
...
Provide an `sf` or `SpatVector` polygon layer whose features define the block
boundaries (e.g. states, counties, hydrological units). The `block_id` argument
names the column that uniquely identifies each feature.

```r
library(sf)

# Example: U.S. state polygons (replace with your own polygon layer)
state_polygons <- sf::read_sf("path/to/states.gpkg")

spdat_poly <- generate_block_sp_index(
  spdat,
...
`generate_cv_index()` returns an integer (or factor) fold-assignment vector with
one value per row of the input. Pass this vector together with your data to
`convert_cv_index_rset()` to obtain a `tidymodels`-compatible `rset` object.

### Leave-one-location-out (lolo)

Each unique location forms one fold; all time steps at that location are held out
together.
...
Each unique time step is one fold; all locations at that time step are held out.
...
Every single row becomes its own fold (standard LOO). Suitable for small datasets.

```r
cv_lolto <- generate_cv_index(spdat, cv_mode = "lolto")
```

### Leave-block-location-out (lblo)

Spatially contiguous groups of locations form each fold. Uses k-means clustering
internally when `cv_fold` is an integer.
...
# You can also supply pre-built polygon blocks:
# cv_lblo_poly <- generate_cv_index(
#   spdat, cv_mode = "lblo",
#   blocks = state_polygons, block_id = "STUSPS"
# )
```

### Leave-block-time-out (lbto)

Consecutive time steps are grouped into `cv_fold` temporal chunks.
...
Plain random k-fold assignment included as a baseline.
...
`plot_cv_folds()` renders a 3-D scatter (lon × lat × time) colored by fold,
useful for quickly checking that the spatial/temporal structure is captured.

```r
if (requireNamespace("plotly", quietly = TRUE)) {
  # Spatial block CV — clusters should appear as distinct spatial patches
  plot_cv_folds(spdat, cv_lblo)

  # Spatiotemporal block CV
  plot_cv_folds(spdat, cv_lblto)
}
```

## Integration with tidymodels

The `rset` objects returned by `convert_cv_index_rset()` are fully compatible with
the `tune` and `workflows` packages.
